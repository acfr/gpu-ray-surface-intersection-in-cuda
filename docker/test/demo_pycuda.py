...
def run_example3(mode):
    ...
    #[Option 1]
    #Comment out the next line if "pycuda_source_legacy.py" derives from commit 8d3efdd.
    params['CUDA_SOURCE_MODULE'] = 'pycuda_source_legacy'
    #This effectively applies the same test twice.
    #- The original intent was to compare the result with legacy code, which could
    #  be problematic for certain device/architecture (as of commit 8d3efdd) since
    #  the legacy script does not contain cleosia's changes (introduced in
    #  "pycuda_source.py" commit 8d3efdd) which addresses memory alignment issues.
    #- This can lead to "pycuda._driver.LogicError: cuMemcpyDtoH failed: an illegal
    #  memory access was encountered" and PyCUDA WARNING: "a clean-up operation
    #  failed (dead context maybe?)"
    #[Option 2]
    #- To fix this for good, in "pycuda_source_legacy_v2.py", the "recursive" device
    #  function bvhUpdateParent is implemented as a loop. Other aspects of the
    #  legacy implementation remain unchanged to facilitate a proper comparison.
    params['CUDA_SOURCE_MODULE'] = 'pycuda_source_legacy_v2'
