//Excerpt from libmorton morton3D.h
//Methods to encode/decode 64-bit morton codes from/to 32-bit (x,y,z) coordinates
//
//Copyright (c) 2016 Jeroen Baert, MIT license
//https://github.com/Forceflow/libmorton/blob/main/LICENSE
//
#pragma once
...
// Magic bits encoding (helper method)
...
inline morton m3D_e_magicbits(const coord x, const coord y, const coord z) {
    return morton3D_SplitBy3bits<morton, coord>(x) |
          (morton3D_SplitBy3bits<morton, coord>(y) << 1) |
          (morton3D_SplitBy3bits<morton, coord>(z) << 2);
}
...
static inline coord morton3D_GetThirdBits(const morton m) {
    morton* masks = (sizeof(morton) <= 4) ?
                     reinterpret_cast<morton*>(magicbit3D_masks32_decode) :
                     reinterpret_cast<morton*>(magicbit3D_masks64_decode);
    morton x = m & masks[5];
    x = (x ^ (x >> 2)) & masks[4];
    x = (x ^ (x >> 4)) & masks[3];
    x = (x ^ (x >> 8)) & masks[2];
